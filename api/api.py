...
async def scrape_and_send_email(driver, browser: str, url: str):
    async with database_manager.get_db() as session:
        try:
            scrap = Scrapper(driver)
...
async def scrape_price(
    data_for_request: RequestData, background_tasks: BackgroundTasks
):
    driver = data_for_request.driver
    browser = data_for_request.browser
    url = data_for_request.url

    background_tasks.add_task(scrape_and_send_email, driver, browser, url)
